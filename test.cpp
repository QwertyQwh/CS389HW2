...
void Collision_test()
{
	...
	delete[] my_value1;
	delete[] my_value2;
	assert(my_cache->get(my_key1, my_size)[0] == 'a');
	assert(my_cache->get(my_key2, my_size)[0] == 'd');
	delete my_cache;
	return;
}
...
void fifo_evictor_test()
{
	...
	my_value1[2] = 'f';
	my_cache->set(my_key1, my_value1, 3);
	assert(my_cache->get(std::to_string(0), my_size) == nullptr);
	assert(my_cache->get(my_key1, my_size)[0] == 'd');
	...
	assert(my_cache->get(std::to_string(2), my_size) == nullptr);
	assert(my_cache->get(std::to_string(1), my_size)[0] == 'd');
	assert(my_cache->space_used() == 150);
	delete[] my_value1;
	delete my_cache;
